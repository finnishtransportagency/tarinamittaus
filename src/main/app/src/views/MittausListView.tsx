...
const MittausListView: React.FC = (): JSX.Element => {
    ...
    const filteredData = mittausData.filter((mittaus) =>
        mittaus.kohde_id.toString().includes(searchText) ||
        (mittaus.katuosoite && mittaus.katuosoite.toLowerCase().includes(searchText.toLowerCase()))
    );
    ...
    return (
        <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '5px', paddingTop: '20px' }}>
                <FormControl
                    type="text"
            ...
            <Table striped>
                ...
                <tbody>
                {filteredData.slice(startIndex, endIndex).map((mittaus: IMittaus, idx: number) => (
                    <tr key={idx}>
                        <td>
                            <Link to={`/mittaus/${mittaus.kohde_id}`}>{mittaus.kohde_id}</Link>
                        </td>
                        <td>
